...
JiaYuan是我程序设计实践的小学期作业 它集成了RAG本地知识库，oj自动刷题，签到监测，ddl看板等功能
旨在利用刚学习的python做一些有趣尝试的同时 便利一点生活 绝无挑战规则之意 
本人将在后续推出移动端并新增更多功能
...
首次启动会把私有运行时释放到当前 Windows 用户的数据目录，耗时通常比后续启动长；基础向量模型已经随安装包提供。桌面数据默认保存在：
